...
static inline model tobj_tomodel(objraw* raw){
	...
	model ret = initmodel();
	ret.npoints = 0;
	ret.d= static_cast<vec3 *>(malloc(sizeof(vec3) * raw->nfaces));
	if(raw->normals)ret.n= static_cast<vec3 *>(malloc(sizeof(vec3) * raw->nfaces));
	if(raw->texcoords)ret.t= static_cast<vec3 *>(malloc(sizeof(vec3) * raw->nfaces));
	if(raw->colors)ret.c= static_cast<vec3 *>(malloc(sizeof(vec3) * raw->nfaces));
	long piter = 0;
...
//Only loads
//static inline objraw tobj_load_buf(std::vector<std::string> array) {
static inline objraw tobj_load_buf(char *buf) {
	objraw retval = initobjraw();

#define TOBJ_PUSH(type, vec, n, val){vec = (type*)realloc(vec, sizeof(type) * (n+1)); vec[n++] = val;}
